...
```
├── .skills/         # Project-scoped AEM EDS agent skills (tool-agnostic); each subfolder is a skill with `SKILL.md`. For Cursor, add a symlink: `.cursor/skills` → `../.skills` so the IDE loads them as project skills.
